...
export class BalancesAccountStorage {
   ...
   *  NOTE: This is only used in the case that this pallet is used to store balances.
   */
  async getAsV2000(key: Uint8Array): Promise<v2000.AccountData> {
    assert(this.isV2000)
    return this._chain.getStorage(this.blockHash, 'Balances', 'Account', key)
  }

  async getManyAsV2000(keys: Uint8Array[]): Promise<(v2000.AccountData)[]> {
    assert(this.isV2000)
    return this._chain.queryStorage(this.blockHash, 'Balances', 'Account', keys.map(k => [k]))
  }

  async getAllAsV2000(): Promise<(v2000.AccountData)[]> {
    assert(this.isV2000)
...
export class GeneralCouncilMembersStorage {
   ...
   *  The current members of the collective. This is stored sorted (just by value).
   */
  async getAsV2000(): Promise<Uint8Array[]> {
    assert(this.isV2000)
...
export class SystemAccountStorage {
   ...
   *  The full account information for a particular account ID.
   */
  async getAsV2000(key: Uint8Array): Promise<v2000.AccountInfo> {
    assert(this.isV2000)
    return this._chain.getStorage(this.blockHash, 'System', 'Account', key)
  }

  async getManyAsV2000(keys: Uint8Array[]): Promise<(v2000.AccountInfo)[]> {
    assert(this.isV2000)
    return this._chain.queryStorage(this.blockHash, 'System', 'Account', keys.map(k => [k]))
  }

  async getAllAsV2000(): Promise<(v2000.AccountInfo)[]> {
    assert(this.isV2000)
