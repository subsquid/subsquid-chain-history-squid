...
export class BalancesAccountStorage {
     ...
     *  NOTE: This is only used in the case that this pallet is used to store balances.
     */
    async getAsV49(key: Uint8Array): Promise<v49.AccountData> {
        assert(this.isV49)
        return this._chain.getStorage(this.blockHash, 'Balances', 'Account', key)
    }

    async getManyAsV49(keys: Uint8Array[]): Promise<(v49.AccountData)[]> {
        assert(this.isV49)
        return this._chain.queryStorage(this.blockHash, 'Balances', 'Account', keys.map(k => [k]))
    }

    async getAllAsV49(): Promise<(v49.AccountData)[]> {
        assert(this.isV49)
...
export class SystemAccountStorage {
     ...
     *  The full account information for a particular account ID.
     */
    async getAsV49(key: Uint8Array): Promise<v49.AccountInfo> {
        assert(this.isV49)
        return this._chain.getStorage(this.blockHash, 'System', 'Account', key)
    }

    async getManyAsV49(keys: Uint8Array[]): Promise<(v49.AccountInfo)[]> {
        assert(this.isV49)
        return this._chain.queryStorage(this.blockHash, 'System', 'Account', keys.map(k => [k]))
    }

    async getAllAsV49(): Promise<(v49.AccountInfo)[]> {
        assert(this.isV49)
