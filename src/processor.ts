...
function getBalanceSetAccount(ctx: ChainContext, event: Event) {
    ...
    if (data.isV13) {
        return toHex(data.asV13[0])
    } else if (data.isV29) {
        return toHex(data.asV29.who)
    } else {
...
function getTransferAccounts(ctx: ChainContext, event: Event) {
    ...
    if (data.isV13) {
        return [toHex(data.asV13[0]), toHex(data.asV13[1])]
    } else if (data.isV29) {
        return [toHex(data.asV29.from), toHex(data.asV29.to)]
    } else {
        throw new UnknownVersionError(data.constructor.name)
    }
}
...
function getEndowedAccount(ctx: ChainContext, event: Event) {
    ...
    if (data.isV13) {
        ...
        return toHex(data.asV29.account)
    } else {
        throw new UnknownVersionError(data.constructor.name)
    }
}
