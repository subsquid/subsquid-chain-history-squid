...
function getBalanceSetAccount(ctx: ChainContext, event: Event) {
    ...
    if (data.isV1000) {
        return toHex(data.asV1000[0])
    } else if (data.isV2010) {
        return toHex(data.asV2010.who)
    } else {
...
function getTransferAccounts(ctx: ChainContext, event: Event) {
    ...
    if (data.isV1000) {
        return [toHex(data.asV1000[0]), toHex(data.asV1000[1])]
    } else if (data.isV2010) {
        return [toHex(data.asV2010.from), toHex(data.asV2010.to)]
    } else {
        throw new UnknownVersionError(data.constructor.name)
    }
}
...
function getEndowedAccount(ctx: ChainContext, event: Event) {
    ...
    if (data.isV1000) {
        ...
        return toHex(data.asV2010.account)
    } else {
        throw new UnknownVersionError(data.constructor.name)
    }
}
