...
function getBalanceSetAccount(ctx: ChainContext, event: Event) {
    ...
    if (data.isV2000) {
        return toHex(data.asV2000[0])
    } else if (data.isV2011) {
        return toHex(data.asV2011.who)
    } else {
...
function getTransferAccounts(ctx: ChainContext, event: Event) {
    ...
    if (data.isV2000) {
        return [toHex(data.asV2000[0]), toHex(data.asV2000[1])]
    } else if (data.isV2011) {
        return [toHex(data.asV2011.from), toHex(data.asV2011.to)]
    } else {
        throw new UnknownVersionError(data.constructor.name)
    }
}
...
function getEndowedAccount(ctx: ChainContext, event: Event) {
    ...
    if (data.isV2000) {
        ...
        return toHex(data.asV2011.account)
    } else {
        throw new UnknownVersionError(data.constructor.name)
    }
}
